    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-6 sm:p-8 max-w-sm w-full">
          ...
          <p className="text-gray-600 mt-4 text-center">Loading PDFs...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="mb-6 sm:mb-8">
            <label className="block">
              ...
              />
              <div className={`
                bg-white rounded-xl shadow-lg p-6 sm:p-8 border-2 border-dashed border-gray-300
                hover:border-blue-400 hover:shadow-xl transition-all cursor-pointer touch-target
                ${uploading || processing ? 'opacity-75 cursor-not-allowed' : ''}
              `}>
                <div className="text-center">
                  <svg className="w-10 h-10 sm:w-12 sm:h-12 text-blue-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {pdfs.length > 0 && (
            <div>
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-4 sm:mb-6">Your Documents</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                {pdfs.map((pdf) => (
                  <div
                    key={pdf.id}
                    onClick={() => handlePdfSelect(pdf.id)}
                    className="bg-white rounded-xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-shadow cursor-pointer border border-gray-200 hover:border-blue-300 touch-target"
                  >
                    <div className="flex items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2 truncate text-responsive">
                          {pdf.title}
                        </h3>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          {pdf.hasChunks ? (
                            ...
                            <button
                              ...
                              className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 hover:bg-yellow-200 transition-colors touch-target"
                            >
                              ⏳ Click to process
                            </button>
                          )}
          ...
          {pdfs.length === 0 && !loading && (
            <div className="text-center py-8 sm:py-12">
              ...
              <p className="text-sm sm:text-base text-gray-600">Upload your first PDF to get started with AI-powered document chat.</p>
            </div>
          )}
        </div>
      </div>
